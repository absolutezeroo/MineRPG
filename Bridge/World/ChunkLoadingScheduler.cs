...
/// Budget: processes at most MaxChunksPerFrame applications per _Process call.
...
public sealed partial class ChunkLoadingScheduler : Node
{
	...
	public override void _Ready()
	{
		...
		if (ServiceLocator.Instance.TryGet<ChunkPersistenceService>(out var persistence))
	...
	{
		var applied = 0;
		while (applied < MaxChunksPerFrame && _readyQueue.TryDequeue(out var entry))
		{
	...
	{
		var needed = _chunkManager.GetCoordsInRange(center, RenderDistance);
		var neededSet = new HashSet<ChunkCoord>(needed);

		foreach (var entry in _chunkManager.GetAll().ToList())
		{
		...
		{
			var entry = _chunkManager.GetOrCreate(coord);
	...
	private void ScheduleChunk(ChunkEntry entry)
	{
		...
		Task.Run(() =>
		{
			try
			{
				...
				var loaded = _persistence?.TryLoad(entry.Coord, entry.Data) ?? false;
			...
			{
				_logger.Error("Chunk generation failed for {0}: {1}", ex, entry.Coord, ex.Message);
			}
			finally
			{
	...
	private void ApplyChunkMesh(ChunkEntry entry)
	{
		...
		var isRemesh = _pendingRemeshes.TryRemove(entry.Coord, out _);

		var chunkNode = _worldNode.GetOrCreateChunkNode(entry.Coord);
	...
	private void ScheduleNeighborRemeshes(ChunkCoord coord)
	{
		...
		{
			if (!_chunkManager.TryGet(neighborCoord, out var neighbor) || neighbor is null)
				continue;
			...
			var capturedNeighbor = neighbor;
			var capturedCoord = neighborCoord;

			Task.Run(() =>
			{
				try
				{
					var neighbors = _chunkManager.GetNeighborData(capturedCoord);
					var mesh = _meshBuilder.Build(capturedNeighbor.Data, neighbors);
				...
				catch (Exception ex)
				{
					...
					_logger.Error("Neighbor remesh failed for {0}: {1}", ex, capturedCoord, ex.Message);
				}
			});
		}
	}
	...
	public void ScheduleBlockEditRemesh(ChunkCoord coord)
	{
		...
		Task.Run(() =>
		{
			...
			{
				var neighbors = _chunkManager.GetNeighborData(capturedCoord);
				var mesh = _meshBuilder.Build(capturedEntry.Data, neighbors);
			...
			{
				_pendingRemeshes.TryRemove(capturedCoord, out _);
				_logger.Error("Block edit remesh failed for {0}: {1}", ex, capturedCoord, ex.Message);
			}
		});
	}
	...
	{
		if (_pendingCts.TryRemove(coord, out var cts))
			cts.Cancel();

		// Save dirty chunk before unloading
		if (_persistence is not null && _chunkManager.TryGet(coord, out var entry) && entry is not null)
	...
	private void SaveAllDirtyChunks()
	{
		...
		var saved = 0;
		foreach (var entry in _chunkManager.GetAll())
		{
